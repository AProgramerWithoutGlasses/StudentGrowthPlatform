...
func OutputMultipleTeacherControl(c *gin.Context) {
	...
	// 处理业务
	excelData, err := service.GetSelectedTeacherExcel(input.SelectedTeachers)
	if err != nil {
	...
	// 响应
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=\"%s\"", url.QueryEscape("批量导出老师信息.xlsx")))
